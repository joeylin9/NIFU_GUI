...
class NIFU_Synthesis:
    def __init__(self):
        ...
        canvas = tk.Canvas(self.root, bd=0, highlightthickness=0, yscrollcommand=vscrollbar.set,
        ...
        for i, pump_name in enumerate(self.pumps_list):
            ...
            # Connect buttons
            pump_connect_button = tk.Button(pumps_frame, text='Disconnected', width=12,
            ...
            pump_set_flow_rate_button = tk.Button(pumps_frame, text='Set', width=5,
                                                  command=lambda i=i: self.pump_set_flow_rate(i))
            pump_set_flow_rate_button.grid(row=i + 1, column=5)

            # use pid or no
            data_types_checkbox = tk.Checkbutton(pumps_frame, text='PID', variable=self.pid_vars[i],
        ...
        tk.Label(equipment_frame, text="3-Way Valves", font=('Arial', 16, 'underline')).pack(anchor='nw', padx=15,
                                                                                             pady=(10, 0))
        self.valves_dict = {
        ...
        tk.Label(equipment_frame, text="Reactor Temperatures (°C)", font=('Arial', 16, 'underline')).pack(anchor='nw',
                                                                                                          padx=15,
                                                                                                          pady=(10, 0))
        ...
        self.temp_connect_button = tk.Button(temps_frame, text="Connect", font=('Arial', 12, 'bold'), width=12,
        ...
        for i, temp_name in enumerate(self.temps_list):
            ...
            temp_entry = tk.Button(temps_frame, text='Set', width=5, command=self.set_temp(i))
        ...
        tk.Label(equipment_frame, text="Liquid Levels (mL)", font=('Arial', 16, 'underline')).pack(anchor='nw', padx=15,
                                                                                                   pady=(10, 0))
        ...
        tk.Label(equipment_frame, text="Stirrer (rpm)", font=('Arial', 16, 'underline')).pack(anchor='nw', padx=15,
                                                                                              pady=(10, 0))
        ...
        tk.Label(data_frame, text="Graph Data", font=('Arial', 16, 'underline')).grid(row=0, column=0, pady=10,
                                                                                      sticky='nw')
        ...
        self.data_type_dict_objects = [self.plot_temperatures, self.plot_pressures, self.plot_balances,
        ...
        # buttons
        self.start_button = tk.Button(graph_buttons_table_frame, text='Start', width=20,
                                      command=self.change_start_button)
        self.start_button.grid(row=0, column=0)
        self.stop_button = tk.Button(graph_buttons_table_frame, text='Stop', width=20, activebackground='IndianRed1',
                                     command=self.change_stop_button)
        self.stop_button.grid(row=1, column=0)
        self.start_excel_button = tk.Button(graph_buttons_table_frame, text='Start Reading Data', width=20,
                                            command=self.start_excel)
        self.start_excel_button.grid(row=2, column=0)
        self.stop_excel_button = tk.Button(graph_buttons_table_frame, text='End Reading Data', width=20,
                                           activebackground='IndianRed1', command=self.stop_excel)
        ...
        for index, value in enumerate(self.plot_temperatures):
            plot_temperatures_checkbox = tk.Checkbutton(self.plot_temperatures_frame, text=value,
        ...
        for index, value in enumerate(self.plot_pressures):
            plot_pressures_checkbox = tk.Checkbutton(self.plot_pressures_frame, text=value,
        ...
        for index, value in enumerate(self.plot_balances):
            plot_balances_checkbox = tk.Checkbutton(self.plot_balances_frame, text=value,
        ...
        for index, value in enumerate(self.plot_flow_rates):
            plot_flow_rates_checkbox = tk.Checkbutton(self.plot_flow_rates_frame, text=value,
    ...
    def pump_connect(self, pump_index):
        if not self.pump_connect_vars[pump_index]:  # If not connected
            ...
            c = pid_control(b_ser, p_ser, self.pump_type_vars[pump_index].get().upper(), self.pumps_list[pump_index],
                            self.g)
            ...
            self.pump_connect_buttons[pump_index].config(bg='SystemButtonFace',
                                                         text='Disconnected')  # Change back to default color
        ...
        for index, label in enumerate(self.current_temp_labels):
            ...
            if reg1:
                ...
                if reg1 != 0 and reg2 != 0:
                    ...
                    t = threading.Thread(target=self.plc.read_temp, args=(self.temps_list[index], label, reg1))
                if t:
                    t.daemon = True
                    t.start()
    ...
    def test(self):
        ...
        self.plc.connect()
        self.plc.reading_onoff(True)
        t = threading.Thread(target=self.plc.read_temp, args=(None, self.current_temp_labels[0], reg1, reg2))
        t.daemon = True
        t.start()
